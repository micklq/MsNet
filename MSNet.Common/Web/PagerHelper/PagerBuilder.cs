    ...
    internal class PagerBuilder
    {
        ...
        private string GenerateAnchor( PagerItem item )
        {
            if ( this._msAjaxPaging )
            {
                ...
                {
                    return this._ajax.RouteLink( item.Text, this._routeName, currentRouteValues, this._ajaxOptions ).ToString();
                }
                return this._ajax.RouteLink( item.Text, currentRouteValues, this._ajaxOptions ).ToString();
            }
        ...
        {
            string str = "";
            if ( this._pagerOptions.ByRawUrl )
            {
                str = GenerateUrlByRawUrl( item.PageIndex );
            }
            else
            {
                str = this.GenerateUrl( item.PageIndex );
            }
        ...
        private string GenerateUrlByRawUrl( int pageIndex )
        {
            ...
            //var query = rawUrl.Query;//?page=3

            var newUri = pageIndex == 1 ?
                rawUrl.BuildNewUri( this._pagerOptions.PageIndexParameterName ) :
                rawUrl.BuildNewUri( this._pagerOptions.PageIndexParameterName, pageIndex.ToString(), this._pagerOptions.PageIndexParameterName );

            var link = newUri.OriginalString;
        ...
        private string GenerateUrl( int pageIndex )
        {
            ...
            RouteValueDictionary
                routeValues = this._routeValues ?? new RouteValueDictionary();

            if ( pageIndex == 0 )
                routeValues[this._pagerOptions.PageIndexParameterName] = "*_MvcPager_PageIndex_*";
            else
                routeValues[this._pagerOptions.PageIndexParameterName] = pageIndex;

            NameValueCollection
                queryString = this._html.ViewContext.HttpContext.Request.QueryString;
            ...
            foreach ( string str in queryString.Keys )
            {
                ...
                {
                    routeValues[str] = queryString[str];
                }
            }
            routeValues["action"] = this._actionName;
            routeValues["controller"] = this._controllerName;

            UrlHelper
                helper = new UrlHelper( this._html.ViewContext.RequestContext );

            if ( !string.IsNullOrEmpty( this._routeName ) )
            {
                return helper.RouteUrl( this._routeName, routeValues );
            }
        ...
        {
            RouteValueDictionary
                dictionary = this._routeValues ?? new RouteValueDictionary();
        ...
        internal MvcHtmlString RenderPager()
        {
            ...
            {
                pagerScript = "if(typeof(jQuery)==\"undefined\"){alert(\"jQuery library not detected, please make sure it loaded prior to MvcPager!\");}";
