...
namespace MSNet.Common.DataRepositories
{
    ...
    public partial class UserPassportRepository : SimpleRepositoryBase<UserPassport, long>, IUserPassportRepository
    {
        ...
        public UserPassport FindByMobile(string mobile)
        {
            var sqlName = this.FormatSqlName("SelectByMobile");
            var sqlParams = new Dictionary<string, object>(1);
            sqlParams.Add("MobilePhone", mobile);
        ...
        /// <returns></returns>
        public bool SignIn(UserPassport passport)
        {
            ArgumentAssertion.IsNotNull(passport, "passport");

            var sqlName = this.FormatSqlName("SignIn");
            var sqlParams = new Dictionary<string, object>(1);
            sqlParams.Add("Id", passport.PassportId);
            sqlParams.Add("LastSignInTime", passport.Profile.LastSignInTime);         
            sqlParams.Add("IsLocked", passport.UserSecurity.IsLocked);
            sqlParams.Add("LastLockedTime", passport.UserSecurity.LastLockedTime);
            sqlParams.Add("FailedPasswordAttemptCount", passport.UserSecurity.FailedPasswordAttemptCount);
